...
タイムチャートはリスト表示と、開始・終了日時を横軸にしたガント表示を切り替えられます。本線とコンチプラン、作業と申請物を区分し、リストではドラッグ＆ドロップで順序を変更できます。担当者は当日体制のメンバーから選択でき、一覧・ガントの開始／完了ボタンでモーダルを開かずに状態と実績日時を更新できます。作業明細の追加時は親作業の作業日と予定時刻を初期表示し、専用の時刻入力と15分・30分・60分の所要時間ボタンで予定を設定できます。各作業には予定開始・終了と実績開始・終了を記録でき、リストとガントで予実を比較できます。ガントには現在時刻を赤い縦線で表示し、1分単位で自動更新します。
...
申請種別管理画面では、全作業で共通利用する申請カテゴリ（資源配布、WFなど）を追加・編集・削除できます。管理画面に追加のACLはなく、`?view=approval-categories`で直接開けます。申請物の種別はマスタ候補から選択するほか、候補外の文字列も任意入力できます。
...
- APIリソース: `/v2/releases`、`/v2/approval-categories`
- データ保存: light-api-serverの `DATA_DIR/releases.json`、`DATA_DIR/approval-categories.json`
