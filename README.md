...
タイムチャートはガントを既定表示とし、リスト表示へ切り替えられます。本線とコンチプラン、作業と申請物を区分し、リストではドラッグ＆ドロップで順序を変更できます。作業明細はタイトルと内容を分けて管理し、種別が申請物の場合は申請物、種別が作業の場合は手順書・関連リンクへ任意に紐づけられます。紐づけ名から対象の詳細を直接開け、ガント左側の作業・体制名からも明細モーダルを開けます。担当者は当日体制のメンバーから選択でき、一覧・ガントの開始／完了ボタンでモーダルを開かずに状態と実績日時を更新できます。ガントではステータスと開始／完了を独立した操作列へまとめます。作業明細の追加時は親作業の作業日と予定時刻を初期表示し、専用の時刻入力と15分・30分・60分の所要時間ボタンで予定を設定できます。各作業には予定開始・終了と実績開始・終了を記録でき、リストとガントで予実を比較できます。ガントには現在時刻を赤い縦線で表示し、タイムチャート内をスクロールしても時間軸ヘッダーと左側の作業・体制列、操作列を固定します。
当日体制では、メンバーごとの対応開始・終了日時、電話番号、場所・拠点に加えて、対応形態を「現地／リモート／オンコール／その他」からラジオで選択できます。対応形態は一覧のラベルとガントバーの色で判別できます。新規追加時は親作業の開始日時から8時間を初期表示します。日跨ぎや複数日の予定にも対応し、作業と同様にバーの移動や両端のドラッグで時間を変更できます。フォームの日時矛盾はモーダル内に理由を表示します。
