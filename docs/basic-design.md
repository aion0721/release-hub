...
| `copyOpen` | 作業コピーモーダルの表示状態 |

詳細表示中はURLの`release`クエリへ作業IDを反映する。初回表示と`popstate`ではクエリからIDを読み取り、対象作業を取得する。共有URLのコピーにはClipboard APIを使用する。

作業コピーはコピー元と新しい親作業日時の差を分単位で計算して工程・体制の予定日時へ加算し、日付の差を申請期限へ加算する。実績日時を空、工程状態を未着手、申請状態を未申請、申請リンクを空文字へ初期化してから、汎用POSTで新規ReleaseRecordとして保存する。
...
- SystemIDとプロジェクト番号の候補は取得済みサマリーから生成し、`input`と`datalist`の組み合わせで候補選択と自由入力を両立する。
- 入力値は前後空白と大文字小文字を無視し、取得済みサマリーへ部分一致で適用する。
...
- ガントを開いている間は算出済みの表示範囲を保持し、データ更新時は必要な方向への拡張だけを行う。端のバーを縮めた際に時間軸自体が再縮小して見かけの幅が戻ることを防ぐ。
- 予定バーと体制バーの幅は期間を表示範囲で除した比率をそのまま使用し、最短5分の表示を固定パーセント幅で上書きしない。
...
- ガントスクロール領域は縦横スクロールを持ち、左上見出しと時間軸を`position: sticky`で上端に固定する。作業・体制列と操作列も`left`を分けて固定し、時間軸だけを横スクロールさせる。
- ガントは「作業・体制」「操作」「時間軸」の3列で構成し、操作列へステータスと開始／完了ボタンを配置する。
...
| `name` | string | Yes | 作業名 |
| `projectNumber` | string | No | プロジェクト番号。未設定時は空文字。旧versionは読み込み時に移行 |
...
| `resourceLinkId` | number | No | kindが作業の場合に紐づけるResourceLinkのID |
