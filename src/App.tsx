    ...
    {ranges.map(({ item, start: savedStart, end: savedEnd, actualStart, actualEnd }) => {
      ...
      return <div className="gantt-row" key={item.id}>
        <div className="gantt-label">
          ...
          <span className="gantt-status-actions"><span className={`tag status-${item.status}`}>{item.status}</span>{item.status === "未着手" && <button type="button" className="timeline-action start" disabled={disabled} onClick={() => onStatusChange(item.id, "start")}>▶ 開始</button>}{item.status === "進行中" && <button type="button" className="timeline-action complete" disabled={disabled} onClick={() => onStatusChange(item.id, "complete")}>✓ 完了</button>}</span>
        </div>
