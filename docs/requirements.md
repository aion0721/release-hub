...
- SystemIDとプロジェクト番号は、取得済み候補からの選択またはテキスト入力で部分一致絞り込みできる。未設定の既存SystemIDは「未設定」として扱う。
- 作業状態を「すべて」「未完了」「完了」で絞り込める。SystemID・プロジェクト番号条件と同時に適用する。
...
- 種別が「作業」の場合、同じ親作業の手順書・関連リンクから任意の1件をIDで紐づけられる。
- 紐づけた手順書名をリストとガントに表示し、クリックすると対象の詳細モーダルを開く。手順書を削除した場合は紐づけだけを解除する。
- 実績開始日時と実績終了日時は任意とする。
- 実績終了のみの入力は禁止する。
...
- ガント左側の作業または体制をクリックすると、対象の明細モーダルを開く。
- タイムチャート内を縦にスクロールしても時間軸ヘッダーを上端に固定する。
- 横スクロール時は作業・体制列と操作列を左側に固定する。
- ステータスと開始／完了ボタンは作業情報から分離した操作列に表示する。当日体制行の操作列は空欄とする。
...
- 氏名、電話番号、対応開始日時、対応終了日時、対応形態、場所・拠点、役割・補足を管理する。
- 対応形態は「現地」「リモート」「オンコール」「その他」の4択とし、ラジオボタンで選択する。
- 氏名、開始・終了日時、対応形態は必須とする。
- 電話番号、場所・拠点、役割・補足は任意とする。場所・拠点が空の場合は対応形態を表示用の補完値として使用する。
- 対応形態は一覧のラベル、行のアクセント、ガントバーの色、詳細モーダルで一貫して識別できるようにする。
- 旧データは場所にオンコールを含む場合はオンコール、リモートまたは在宅を含む場合はリモート、それ以外は現地として補完する。
