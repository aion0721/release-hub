...
タイムチャートはリスト表示と、開始・終了日時を横軸にしたガント表示を切り替えられます。本線とコンチプランを区分し、リストではドラッグ＆ドロップで順序を変更できます。各作業には予定開始・終了と実績開始・終了を記録でき、リストとガントで予実を比較できます。実績には現在日時をワンクリックで設定でき、進行中の作業は実績開始のみでも保存できます。ガントには現在時刻を赤い縦線で表示し、1分単位で自動更新します。
