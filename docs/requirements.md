...
- Node.js 20以上を必要とする。
- モダンブラウザを対象とする。
- 本番はNode.jsプロセスからSPAとAPIを同一オリジンで配信できる。

### 8.2 セキュリティ

- API応答に `X-Content-Type-Options: nosniff`、`X-Frame-Options: SAMEORIGIN`、`Referrer-Policy: same-origin` を付与する。
- CORSは `CORS_ORIGIN` が設定された場合のみ許可する。
- リクエストボディは1,000,000文字を上限とする。
- 認証・認可は社内リバースプロキシまたはSSOで実施する前提とする。

### 8.3 可用性・整合性

- JSON書き込みはプロセス内キューで直列化する。
- 一時ファイルへ書き込み後にrenameして置換する。
...
- APIはフィールド単位の厳密なスキーマ検証を行わず、主に親構造を検証する。
