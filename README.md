...
タイムチャートはガントを既定表示とし、リスト表示へ切り替えられます。本線とコンチプラン、作業と申請物を区分し、リストではドラッグ＆ドロップで順序を変更できます。作業明細はタイトルと内容を分けて管理し、種別が申請物の場合は申請物一覧の項目へ任意に紐づけられ、表示した申請名から詳細モーダルを直接開けます。担当者は当日体制のメンバーから選択でき、一覧・ガントの開始／完了ボタンでモーダルを開かずに状態と実績日時を更新できます。作業明細の追加時は親作業の作業日と予定時刻を初期表示し、専用の時刻入力と15分・30分・60分の所要時間ボタンで予定を設定できます。各作業には予定開始・終了と実績開始・終了を記録でき、リストとガントで予実を比較できます。ガントには現在時刻を赤い縦線で表示し、1分単位で自動更新します。
