...
import type { ApprovalItem, ApprovalStatus, Category, CreateReleaseInput, ReleaseSummary, ReleaseWork, ResourceLink, StaffingAssignment, StaffingMode, TimelineItem, TimelineKind, TimelinePlan, TimelineStatus } from "./types";

type ModalType = "work" | "staffing" | "timeline" | "approval" | "link";
type PreviewItem = { type: "timeline"; item: TimelineItem } | { type: "staffing"; item: StaffingAssignment } | { type: "approval"; item: ApprovalItem } | { type: "link"; item: ResourceLink };
...
type DetailTarget = Exclude<EditTarget, { type: "work" }>;
...
function normalizeStaffingMode(mode?: string, location = ""): StaffingMode {
  if ((["現地", "リモート", "オンコール", "その他"] as StaffingMode[]).includes(mode as StaffingMode)) return mode as StaffingMode;
  if (/オンコール/i.test(location)) return "オンコール";
  if (/リモート|在宅/i.test(location)) return "リモート";
  return "現地";
}

function staffingModeIcon(mode: StaffingMode) {
  return mode === "現地" ? "⌂" : mode === "リモート" ? "⌁" : mode === "オンコール" ? "☏" : "◇";
}
...
function normalizeWork(work: ReleaseWork): ReleaseWork {
  return {
    ...
    timeline: (work.timeline || []).map((item) => ({
      ...item,
      content: item.content || "",
      kind: item.kind || "作業",
      approvalId: item.kind === "申請物" && Number.isInteger(item.approvalId) ? item.approvalId : undefined,
      resourceLinkId: (item.kind || "作業") === "作業" && Number.isInteger(item.resourceLinkId) ? item.resourceLinkId : undefined,
    })),
    staffing: (work.staffing || []).map((item) => ({ ...item, workMode: normalizeStaffingMode(item.workMode, item.location) })),
    approvals: (work.approvals || []).map((item) => ({ ...item, category: item.category || "", due: normalizeDueDate(item.due, work.release.releaseDate), status: normalizeApprovalStatus(item.status), note: item.note || "" })),
    links: (work.links || []).map((item) => ({ ...item, note: item.note || "" })),
  };
}
...
export default function App() {
  ...
  const [detailDeleteTarget, setDetailDeleteTarget] = useState<DetailTarget | null>(null);
  ...
  function commit(nextWork: ReleaseWork, section: ReleaseWorkSection, allowEmptyTimeline = false) {
    ...
    if (!allowEmptyTimeline && section === "timeline" && previous.timeline.length > 0 && consolidated.timeline.length === 0) {
  ...
  function updateApprovalStatus(id: number, status: ApprovalStatus) {
    const current = selectedRef.current;
    if (!current) return;
    const approvals = current.approvals.map((item) => item.id === id ? { ...item, status } : item);
    commit({ ...current, approvals }, "approvals");
  }

  function requestDetailDelete(target: DetailTarget) {
    setPreview(null);
    closeModal();
    setDetailDeleteTarget(target);
  }

  function deleteDetail() {
    const target = detailDeleteTarget;
    const current = selectedRef.current;
    if (!target || !current) return;
    setDetailDeleteTarget(null);
    if (target.type === "timeline") {
      commit({ ...current, timeline: current.timeline.filter((item) => item.id !== target.item.id) }, "timeline", true);
    } else if (target.type === "staffing") {
      commit({ ...current, staffing: current.staffing.filter((item) => item.id !== target.item.id) }, "staffing");
    } else if (target.type === "approval") {
      const timeline = current.timeline.map((item) => item.approvalId === target.item.id ? { ...item, approvalId: undefined } : item);
      commit({ ...current, approvals: current.approvals.filter((item) => item.id !== target.item.id) }, "approvals");
      if (timeline.some((item, index) => item !== current.timeline[index])) {
        const latest = selectedRef.current;
        if (latest) commit({ ...latest, timeline }, "timeline", true);
      }
    } else {
      const timeline = current.timeline.map((item) => item.resourceLinkId === target.item.id ? { ...item, resourceLinkId: undefined } : item);
      commit({ ...current, links: current.links.filter((item) => item.id !== target.item.id) }, "links");
      if (timeline.some((item, index) => item !== current.timeline[index])) {
        const latest = selectedRef.current;
        if (latest) commit({ ...latest, timeline }, "timeline", true);
      }
  ...
  async function submitCopy(event: FormEvent<HTMLFormElement>) {
    ...
    const input: CreateReleaseInput = {
      ...
      projectNumber: String(values.projectNumber),
  ...
  async function submitItem(event: FormEvent<HTMLFormElement>) {
    ...
    if (modal === "staffing") {
      ...
      const workMode = normalizeStaffingMode(String(values.workMode || ""), String(values.location || ""));
      const item: StaffingAssignment = { id: editing?.id ?? nextId(selected.staffing), name: String(values.name), phone: String(values.phone || ""), startAt: String(values.startAt), endAt: String(values.endAt), workMode, location: String(values.location || workMode), note: String(values.note || "") };
      ...
      const kind = String(values.kind || "作業") as TimelineKind;
      const approvalId = Number(values.approvalId);
      const resourceLinkId = Number(values.resourceLinkId);
      const item: TimelineItem = { id: editing?.id ?? nextId(selected.timeline), startAt: String(values.startAt), endAt: String(values.endAt), actualStartAt: String(values.actualStartAt || ""), actualEndAt: String(values.actualEndAt || ""), title: String(values.title), content: String(values.content || ""), owner: String(values.owner), status: String(values.status || "未着手") as TimelineStatus, plan: String(values.plan || "本線") as TimelinePlan, kind, approvalId: kind === "申請物" && Number.isInteger(approvalId) && approvalId > 0 ? approvalId : undefined, resourceLinkId: kind === "作業" && Number.isInteger(resourceLinkId) && resourceLinkId > 0 ? resourceLinkId : undefined };
      ...
      const item: ApprovalItem = { id: editing?.id ?? nextId(selected.approvals), title: String(values.title), category: String(values.category || ""), owner: String(values.owner), due: String(values.due), status: String(values.status || "未申請") as ApprovalItem["status"], url: String(values.url), note: String(values.note || "") };
      ...
      const item: ResourceLink = { id: editing?.id ?? nextId(selected.links), title: String(values.title), category: String(values.category), description: String(values.description), url: String(values.url), note: String(values.note || "") };
    ...
    <main className="app-shell">
      ...
      <section className="content">
        ...
        {adminOpen ? (
          ...
          <WorkDetail work={selected} loading={loading} saving={saving} onBack={() => showList()} onRefresh={() => void openWork(selected.release.id, "none")} onCopy={() => setCopyOpen(true)} onDelete={() => setDeleteConfirmOpen(true)} onOpenModal={openModal} onOpenEditor={openEditor} onOpenPreview={setPreview} onReorderTimeline={reorderTimeline} onUpdateTimelineTime={updateTimelineTime} onUpdateStaffingTime={updateStaffingTime} onUpdateTimelineStatus={updateTimelineStatus} onUpdateApprovalStatus={updateApprovalStatus} />
        ) : (
          <WorkList summaries={summaries} loading={loading} onCreate={() => openModal("work")} onRefresh={() => void loadSummaries()} onOpen={openWork} />
        )}
      </section>
      {modal && <ItemModal type={modal} editTarget={editTarget} releaseDate={selected?.release.releaseDate} staffing={selected?.staffing || []} approvals={selected?.approvals || []} links={selected?.links || []} approvalCategories={approvalCategories} formError={formError} saving={saving} onClose={closeModal} onSubmit={submitItem} onRequestDelete={requestDetailDelete} />}
      {preview && <PreviewModal preview={preview} onClose={() => setPreview(null)} onEdit={(target) => openEditor(target)} onRequestDelete={requestDetailDelete} />}
      ...
      {detailDeleteTarget && <DetailDeleteConfirmModal target={detailDeleteTarget} saving={saving} onClose={() => setDetailDeleteTarget(null)} onConfirm={deleteDetail} />}
    </main>
  );
}
...
function WorkList({ summaries, loading, onCreate, onRefresh, onOpen }: { summaries: ReleaseSummary[]; loading: boolean; onCreate: () => void; onRefresh: () => void; onOpen: (id: number) => void }) {
  ...
  const [groupMode, setGroupMode] = useState<"none" | "projectNumber">("none");
  ...
  const projectNumbers = useMemo(() => [...new Set(summaries.map((item) => item.projectNumber).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ja")), [summaries]);
  const identityFiltered = useMemo(() => {
    const systemQuery = systemFilter.trim().toLocaleLowerCase("ja");
    const projectQuery = projectFilter.trim().toLocaleLowerCase("ja");
    return summaries.filter((item) => (!systemQuery || item.systemId.toLocaleLowerCase("ja").includes(systemQuery)) && (!projectQuery || item.projectNumber.toLocaleLowerCase("ja").includes(projectQuery)));
  }, [summaries, systemFilter, projectFilter]);
  const filtered = useMemo(() => identityFiltered.filter((item) => statusFilter === "all" || (statusFilter === "completed" ? item.status === "完了" : item.status !== "完了")), [identityFiltered, statusFilter]);
  const projectGroups = useMemo(() => {
    const groups = new Map<string, ReleaseSummary[]>();
    for (const work of filtered) {
      const key = work.projectNumber || "プロジェクト番号未設定";
      groups.set(key, [...(groups.get(key) || []), work]);
    }
    return [...groups.entries()].sort(([left], [right]) => left === "プロジェクト番号未設定" ? 1 : right === "プロジェクト番号未設定" ? -1 : left.localeCompare(right, "ja"));
  }, [filtered]);
  const activeCount = identityFiltered.filter((item) => item.status !== "完了").length;
  ...
  return <>
    ...
    <section className="list-hero">
      <div><span className="section-kicker">OVERVIEW</span><strong>{filtered.length}</strong><p>{systemFilter || projectFilter ? "入力条件に一致するリリース作業" : "全SystemIDのリリース作業"}</p></div>
    ...
    <section className="panel work-list-panel">
      <div className="panel-heading work-list-heading"><div><span className="section-kicker">{view === "list" ? "RELEASE QUEUE" : "RELEASE CALENDAR"}</span><h2>{view === "list" ? "作業を選択" : "作業カレンダー"}</h2></div><div className="work-view-controls"><label>SystemID<input className="filter-combobox" list="system-filter-options" value={systemFilter} onChange={(event) => setSystemFilter(event.target.value)} placeholder="すべて／入力して検索" aria-label="SystemIDを入力または候補から絞り込み" /><datalist id="system-filter-options">{systemIds.map((systemId) => <option value={systemId} key={systemId} />)}</datalist></label><label>プロジェクト番号<input className="filter-combobox" list="project-filter-options" value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)} placeholder="すべて／入力して検索" aria-label="プロジェクト番号を入力または候補から絞り込み" /><datalist id="project-filter-options">{projectNumbers.map((projectNumber) => <option value={projectNumber} key={projectNumber} />)}</datalist></label><label>作業状態<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "all" | "active" | "completed")} aria-label="作業状態で絞り込み"><option value="all">すべて</option><option value="active">未完了</option><option value="completed">完了</option></select></label>{view === "list" && <label>グループ<select value={groupMode} onChange={(event) => setGroupMode(event.target.value as "none" | "projectNumber")} aria-label="作業をグループ化"><option value="none">グループなし</option><option value="projectNumber">プロジェクト番号</option></select></label>}<div className="view-switch" aria-label="作業表示"><button className={view === "list" ? "active" : ""} onClick={() => setView("list")} aria-pressed={view === "list"}>☷ リスト</button><button className={view === "calendar" ? "active" : ""} onClick={() => setView("calendar")} aria-pressed={view === "calendar"}>▦ カレンダー</button></div><span className="list-count">{filtered.length}件</span></div></div>
      {view === "list" ? <><div className="work-table-head"><span>作業名</span><span>SystemID</span><span>作業日時</span><span>責任者</span><span>進捗</span><span>状態</span><span /></div>{!filtered.length ? <div className="empty-state"><span>＋</span><h3>{summaries.length ? "該当する作業はありません" : "最初の作業を登録しましょう"}</h3><p>{summaries.length ? "SystemID、プロジェクト番号または作業状態の絞り込み条件を変更してください。" : "登録後にタイムチャートや申請物を追加できます。"}</p>{!summaries.length && <button className="primary-button" onClick={onCreate}>作業を登録</button>}</div> : groupMode === "projectNumber" ? <div className="project-groups">{projectGroups.map(([projectNumber, works]) => <section className="project-group" key={projectNumber}><div className="project-group-heading"><span>PROJECT</span><strong>{projectNumber}</strong><small>{works.length}件</small></div><div className="work-list"><WorkRows works={works} loading={loading} onOpen={onOpen} /></div></section>)}</div> : <div className="work-list"><WorkRows works={filtered} loading={loading} onOpen={onOpen} /></div>}</> : <div className="release-calendar"><div className="calendar-toolbar"><button type="button" onClick={() => setCalendarMonth((month) => shiftMonth(month, -1))} aria-label="前の月">‹</button><strong>{formatMonth(calendarMonth)}</strong><button type="button" onClick={() => setCalendarMonth((month) => shiftMonth(month, 1))} aria-label="次の月">›</button></div><div className="calendar-weekdays">{["日", "月", "火", "水", "木", "金", "土"].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarDays.map((day) => { const works = filtered.filter((work) => work.releaseDate.slice(0, 10) === day.value); return <div className={`calendar-day ${day.inMonth ? "" : "outside"}`} key={day.value}><span className="calendar-day-number">{day.day}</span><div className="calendar-events">{works.map((work) => <button type="button" key={work.id} onClick={() => onOpen(work.id)} disabled={loading} aria-label={`${work.name}の詳細を開く`}><span>{work.releaseDate.slice(11, 16)}</span><strong>{work.name}</strong><small>{work.systemId}{work.projectNumber ? `・${work.projectNumber}` : ""}</small></button>)}</div></div>; })}</div></div>}
    </section>
  </>;
}

function WorkRows({ works, loading, onOpen }: { works: ReleaseSummary[]; loading: boolean; onOpen: (id: number) => void }) {
  return <>{works.map((work) => <button className="work-row" key={work.id} onClick={() => onOpen(work.id)} disabled={loading}>
    <span className="work-title"><i>{work.name.slice(0, 1)}</i><span><strong>{work.name}</strong><small>{work.projectNumber ? `${work.projectNumber}・` : ""}{work.environment}</small></span></span><span className="system-id-badge">{work.systemId}</span>
...
function WorkDetail({ work, loading, saving, onBack, onRefresh, onCopy, onDelete, onOpenModal, onOpenEditor, onOpenPreview, onReorderTimeline, onUpdateTimelineTime, onUpdateStaffingTime, onUpdateTimelineStatus, onUpdateApprovalStatus }: { work: ReleaseWork; loading: boolean; saving: boolean; onBack: () => void; onRefresh: () => void; onCopy: () => void; onDelete: () => void; onOpenModal: (type: ModalType) => void; onOpenEditor: (target: EditTarget) => void; onOpenPreview: (preview: PreviewItem) => void; onReorderTimeline: (sourceId: number, targetId: number | null, targetPlan: TimelinePlan) => void; onUpdateTimelineTime: (id: number, startAt: string, endAt: string) => void; onUpdateStaffingTime: (id: number, startAt: string, endAt: string) => void; onUpdateTimelineStatus: (id: number, action: "start" | "complete") => void; onUpdateApprovalStatus: (id: number, status: ApprovalStatus) => void }) {
  const [timelineView, setTimelineView] = useState<"list" | "gantt">("gantt");
  ...
  return <>
    ...
    <div className="workspace-grid"><section id="timeline" className={`panel timeline-panel ${timelineView === "gantt" ? "gantt-panel" : ""}`}><div className="panel-heading"><div><span className="section-kicker">ALL-IN-ONE</span><h2>タイムチャート</h2><p className="timeline-drag-hint">{timelineView === "list" ? "作業と当日体制を一覧で編集。作業行は上下にドラッグ可能" : "作業と当日体制を同じ時間軸で表示。各バーは5分単位でドラッグ変更"}</p></div><div className="panel-actions"><div className="view-switch" aria-label="オールインワン表示"><button className={timelineView === "list" ? "active" : ""} onClick={() => setTimelineView("list")} aria-pressed={timelineView === "list"}>☷ リスト</button><button className={timelineView === "gantt" ? "active" : ""} onClick={() => setTimelineView("gantt")} aria-pressed={timelineView === "gantt"}>▥ ガント</button></div><button className="staffing-button compact-button" onClick={() => onOpenModal("staffing")}>＋ 体制</button><button className="primary-button compact-button" onClick={() => onOpenModal("timeline")}>＋ 作業</button></div></div>{timelineView === "list" ? <AllInOneList items={work.timeline} assignments={work.staffing} approvals={work.approvals} links={work.links} disabled={loading || saving} onEditTimeline={(item) => onOpenEditor({ type: "timeline", item })} onEditStaffing={(item) => onOpenEditor({ type: "staffing", item })} onOpenApproval={(item) => onOpenPreview({ type: "approval", item })} onOpenResource={(item) => onOpenPreview({ type: "link", item })} onReorder={onReorderTimeline} onStatusChange={onUpdateTimelineStatus} /> : <GanttChart items={work.timeline} assignments={work.staffing} approvals={work.approvals} links={work.links} disabled={loading || saving} onEdit={(item) => onOpenEditor({ type: "timeline", item })} onEditStaffing={(item) => onOpenEditor({ type: "staffing", item })} onOpenPreview={onOpenPreview} onTimeChange={onUpdateTimelineTime} onStaffingTimeChange={onUpdateStaffingTime} onStatusChange={onUpdateTimelineStatus} />}</section>
      <section id="approvals" className="panel approvals-panel"><div className="panel-heading"><div><span className="section-kicker">APPROVALS</span><h2>申請物一覧</h2></div><button className="ghost-button" onClick={() => onOpenModal("approval")}>＋ 追加</button></div><div className="approval-list">{work.approvals.map((item) => <article key={item.id} className="approval-row"><button type="button" className="approval-detail-button" onClick={() => onOpenPreview({ type: "approval", item })} aria-label={`${item.title}の詳細を開く`}><span className={`approval-status-icon status-${item.status}`} aria-hidden="true">{item.status === "未申請" ? "○" : item.status === "申請中" ? "↗" : item.status === "回付済" ? "⇢" : "✓"}</span><span><strong>{item.title}</strong><small>{item.category ? <b className="approval-category-badge">{item.category}</b> : null}{item.owner}・期限 {formatDueDate(item.due)}</small>{item.note && <small className="item-note">備考：{item.note}</small>}</span><span className="external-link">詳細 ›</span></button><div className="approval-quick-actions"><select value={item.status} onChange={(event) => onUpdateApprovalStatus(item.id, event.target.value as ApprovalStatus)} disabled={loading || saving} aria-label={`${item.title}のステータス`}><option>未申請</option><option>申請中</option><option>回付済</option><option>結了済</option></select>{item.url ? <a href={item.url} target="_blank" rel="noopener noreferrer">開く ↗</a> : <span>URL未登録</span>}</div></article>)}{!work.approvals.length && <p className="section-empty">まだ申請物はありません</p>}</div></section></div>
    <section id="links" className="panel links-panel"><div className="panel-heading"><div><span className="section-kicker">RESOURCES</span><h2>手順書・関連リンク</h2></div><button className="ghost-button" onClick={() => onOpenModal("link")}>＋ 追加</button></div><div className="link-grid">{work.links.map((item) => <article key={item.id} className="link-card"><button type="button" className="link-card-detail" onClick={() => onOpenPreview({ type: "link", item })} aria-label={`${item.title}の詳細を開く`}><span className="doc-icon">▤</span><span><small>{item.category}</small><strong>{item.title}</strong><p>{item.description}</p>{item.note && <p className="item-note">備考：{item.note}</p>}</span><b>›</b></button>{item.url ? <a className="link-direct-button" href={item.url} target="_blank" rel="noopener noreferrer">そのまま開く ↗</a> : <span className="link-direct-button disabled">URL未登録</span>}</article>)}</div>{!work.links.length && <p className="section-empty links-empty">まだリンクはありません</p>}</section>
    <footer>最終更新：{work.release.updatedAt || "未更新"} ・ {work.release.updatedBy}</footer>
  </>;
}

function linkedApproval(item: TimelineItem, approvals: ApprovalItem[]) {
  return item.kind === "申請物" && item.approvalId ? approvals.find((approval) => approval.id === item.approvalId) : undefined;
}

function linkedResource(item: TimelineItem, links: ResourceLink[]) {
  return (item.kind || "作業") === "作業" && item.resourceLinkId ? links.find((link) => link.id === item.resourceLinkId) : undefined;
}

function TimelineList({ items, approvals, links, disabled, onEdit, onOpenApproval, onOpenResource, onReorder, onStatusChange }: { items: TimelineItem[]; approvals: ApprovalItem[]; links: ResourceLink[]; disabled: boolean; onEdit: (item: TimelineItem) => void; onOpenApproval: (item: ApprovalItem) => void; onOpenResource: (item: ResourceLink) => void; onReorder: (sourceId: number, targetId: number | null, targetPlan: TimelinePlan) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
  ...
  return <div className="timeline-list">{plans.map((plan) => {
    ...
    return <section className={`timeline-group plan-${plan}`} key={plan} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const sourceId = Number(event.dataTransfer.getData("text/plain")) || draggingId; if (sourceId) onReorder(sourceId, null, plan); setDraggingId(null); }}><div className="timeline-group-heading"><h3>{plan}</h3><span>{planItems.length}件</span></div>
      {planItems.length ? planItems.map((item, index) => { const approval = linkedApproval(item, approvals); const resource = linkedResource(item, links); return <div key={item.id} draggable={!disabled} className={`timeline-row ${item.status === "完了" ? "done" : ""} ${draggingId === item.id ? "dragging" : ""}`} onClick={() => onEdit(item)} onKeyDown={(event) => { if (event.key === "Enter") onEdit(item); }} onDragStart={(event: DragEvent<HTMLDivElement>) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", String(item.id)); setDraggingId(item.id); }} onDragEnd={() => setDraggingId(null)} onDragOver={(event: DragEvent<HTMLDivElement>) => event.preventDefault()} onDrop={(event: DragEvent<HTMLDivElement>) => { event.preventDefault(); event.stopPropagation(); const sourceId = Number(event.dataTransfer.getData("text/plain")) || draggingId; if (sourceId) onReorder(sourceId, item.id, plan); setDraggingId(null); }} role="button" tabIndex={disabled ? -1 : 0} aria-label={`${item.title}を編集`} title="ドラッグして並べ替え。クリックで編集"><span className="drag-handle">⋮⋮</span><span className="time">{formatDateTime(item.startAt)}</span><span className="line"><i>{item.status === "完了" ? "✓" : index + 1}</i></span><span className="task"><strong><b className={`timeline-kind kind-${item.kind || "作業"}`}>{item.kind || "作業"}</b>{item.title}</strong>{item.content && <small className="timeline-content">{item.content}</small>}{approval && <button type="button" className="timeline-approval-link" onClick={(event) => { event.stopPropagation(); onOpenApproval(approval); }}>申請物：{approval.title} ›</button>}{resource && <button type="button" className="timeline-resource-link" onClick={(event) => { event.stopPropagation(); onOpenResource(resource); }}>手順書：{resource.title} ›</button>}<small><b className="time-kind planned">予定</b>担当：{item.owner}・{formatDateTimeRange(item.startAt, item.endAt)}</small><small className={`actual-time ${item.actualStartAt ? "recorded" : ""}`}><b className="time-kind actual">実績</b>{item.actualStartAt ? item.actualEndAt ? formatDateTimeRange(item.actualStartAt, item.actualEndAt) : `${formatDateTime(item.actualStartAt)}–進行中` : "未入力"}</small></span><span className="timeline-row-actions"><span className={`tag status-${item.status}`}>{item.status}</span>{item.status === "未着手" && <button type="button" className="timeline-action start" disabled={disabled} onClick={(event) => { event.stopPropagation(); onStatusChange(item.id, "start"); }}>▶ 開始</button>}{item.status === "進行中" && <button type="button" className="timeline-action complete" disabled={disabled} onClick={(event) => { event.stopPropagation(); onStatusChange(item.id, "complete"); }}>✓ 完了</button>}</span></div>; }) : <p className="timeline-group-empty">ここにドロップして移動</p>}
    </section>;
  })}</div>;
}
...
function AllInOneList({ items, assignments, approvals, links, disabled, onEditTimeline, onEditStaffing, onOpenApproval, onOpenResource, onReorder, onStatusChange }: { items: TimelineItem[]; assignments: StaffingAssignment[]; approvals: ApprovalItem[]; links: ResourceLink[]; disabled: boolean; onEditTimeline: (item: TimelineItem) => void; onEditStaffing: (item: StaffingAssignment) => void; onOpenApproval: (item: ApprovalItem) => void; onOpenResource: (item: ResourceLink) => void; onReorder: (sourceId: number, targetId: number | null, targetPlan: TimelinePlan) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
  return <div className="all-in-one-list">
    <section className="operation-list-section"><div className="operation-subheading"><div><span className="operation-icon work">◷</span><h3>タイムチャート</h3></div><span>{items.length}件</span></div><TimelineList items={items} approvals={approvals} links={links} disabled={disabled} onEdit={onEditTimeline} onOpenApproval={onOpenApproval} onOpenResource={onOpenResource} onReorder={onReorder} onStatusChange={onStatusChange} /></section>
    <section id="staffing" className="operation-list-section"><div className="operation-subheading"><div><span className="operation-icon staffing">♙</span><h3>当日体制</h3></div><span>{assignments.length}名</span></div>
      {!assignments.length ? <p className="section-empty">まだ当日の体制は登録されていません</p> : <div className="staffing-compact-list">{assignments.map((assignment) => <button type="button" className={`staffing-compact-row mode-${assignment.workMode}`} key={assignment.id} onClick={() => onEditStaffing(assignment)} disabled={disabled} aria-label={`${assignment.name}の体制を編集`}><span className="member-avatar">{assignment.name.slice(0, 1)}</span><span className="staffing-person"><span className="staffing-name-line"><strong>{assignment.name}</strong><b className={`staffing-mode-badge mode-${assignment.workMode}`}>{staffingModeIcon(assignment.workMode)} {assignment.workMode}</b></span><small>{assignment.phone || "電話番号未登録"}</small></span><span className="staffing-time"><strong>{formatDateTimeRange(assignment.startAt, assignment.endAt)}</strong><small>{assignment.location || assignment.workMode}{assignment.note ? `・${assignment.note}` : ""}</small></span><span className="row-arrow">›</span></button>)}</div>}
    </section>
  </div>;
}
...
function GanttChart({ items, assignments, approvals, links, disabled, onEdit, onEditStaffing, onOpenPreview, onTimeChange, onStaffingTimeChange, onStatusChange }: { items: TimelineItem[]; assignments: StaffingAssignment[]; approvals: ApprovalItem[]; links: ResourceLink[]; disabled: boolean; onEdit: (item: TimelineItem) => void; onEditStaffing: (item: StaffingAssignment) => void; onOpenPreview: (preview: PreviewItem) => void; onTimeChange: (id: number, startAt: string, endAt: string) => void; onStaffingTimeChange: (id: number, startAt: string, endAt: string) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
  ...
  return <div className="gantt-scroll"><div className={`gantt-chart ${currentTimeVisible ? "now-visible" : ""}`} style={{ minWidth: `${Math.max(850, 350 + duration * 1.35)}px`, "--gantt-now-left": currentTimeLeft } as CSSProperties} role="group" aria-label={`${formatMinutes(rangeStart)}から${formatMinutes(rangeEnd)}までの作業と当日体制`}>
    <div className="gantt-corner">作業・メンバー</div><div className="gantt-action-header">操作</div><div className="gantt-axis">{ticks.map((tick) => <span key={tick} style={{ left: `${((tick - rangeStart) / duration) * 100}%` }}>{formatMinutes(tick)}</span>)}{currentTimeVisible ? <i className="gantt-now-marker" style={{ left: currentTimeLeft }}><b>現在 {formatMinutes(currentMinute)}</b></i> : <em className="gantt-now-outside">現在 {formatMinutes(currentMinute)}（表示範囲外）</em>}</div>
    <div className="gantt-section-label"><span className="operation-icon work">◷</span>作業</div><div className="gantt-section-action" aria-hidden="true" /><div className="gantt-section-line"><span>WORK</span><div className="gantt-legend"><i className="planned" />予定<i className="actual" />実績</div></div>
    {ranges.map(({ item, start: savedStart, end: savedEnd, actualStart, actualEnd }) => {
      ...
      const approval = linkedApproval(item, approvals);
      const resource = linkedResource(item, links);
      return <div className="gantt-row" key={item.id}>
        <div className="gantt-label">
          <button type="button" className="gantt-label-detail" onClick={() => onOpenPreview({ type: "timeline", item })} disabled={disabled} aria-label={`${item.title}の明細を表示`}>
            <strong><b className={`timeline-kind kind-${item.kind || "作業"}`}>{item.kind || "作業"}</b>{item.title}</strong>
            {item.content && <small className="timeline-content">{item.content}</small>}
            <small><b className="time-kind planned">予定</b>{item.plan}・{item.owner}・{formatDateTimeRange(fromMinutes(start), fromMinutes(end))}</small>
            <small className={item.actualStartAt ? "actual-recorded" : ""}><b className="time-kind actual">実績</b>{item.actualStartAt ? item.actualEndAt ? formatDateTimeRange(item.actualStartAt, item.actualEndAt) : `${formatDateTime(item.actualStartAt)}–進行中` : "未入力"}</small>
          </button>
          {approval && <button type="button" className="timeline-approval-link gantt-approval-link" onClick={() => onOpenPreview({ type: "approval", item: approval })}>申請物：{approval.title} ›</button>}
          {resource && <button type="button" className="timeline-resource-link gantt-resource-link" onClick={() => onOpenPreview({ type: "link", item: resource })}>手順書：{resource.title} ›</button>}
        </div>
        <div className="gantt-action-cell"><span className={`tag status-${item.status}`}>{item.status}</span>{item.status === "未着手" && <button type="button" className="timeline-action start" disabled={disabled} onClick={() => onStatusChange(item.id, "start")}>▶ 開始</button>}{item.status === "進行中" && <button type="button" className="timeline-action complete" disabled={disabled} onClick={() => onStatusChange(item.id, "complete")}>✓ 完了</button>}</div>
        <div className="gantt-lane work-gantt-lane" style={{ backgroundSize: `${100 / Math.max(1, ticks.length - 1)}% 100%` }}><div className={`gantt-bar gantt-${item.status} plan-${item.plan} ${dragging ? "dragging" : ""} ${disabled ? "disabled" : ""}`} style={{ left: `${((start - rangeStart) / duration) * 100}%`, width: `${((end - start) / duration) * 100}%` }}><button className="gantt-bar-content" onPointerDown={(event) => beginTimeDrag(event, item.id, "timeline", "move", savedStart, savedEnd)} onPointerMove={moveTimeDrag} onPointerUp={endTimeDrag} onPointerCancel={endTimeDrag} onClick={() => { if (suppressClickRef.current) { suppressClickRef.current = false; return; } onEdit(item); }} disabled={disabled} aria-label={`${item.title}の予定時間帯をドラッグで移動。クリックで編集`} title="左右にドラッグして予定時間帯を移動"><span>{item.status === "完了" ? "✓ " : ""}{item.title}</span></button><button className="gantt-resize-handle start" onPointerDown={(event) => beginTimeDrag(event, item.id, "timeline", "start", savedStart, savedEnd)} onPointerMove={moveTimeDrag} onPointerUp={endTimeDrag} onPointerCancel={endTimeDrag} disabled={disabled} aria-label={`${item.title}の予定開始時刻をドラッグで変更`} title="予定開始時刻を変更" /><button className="gantt-resize-handle end" onPointerDown={(event) => beginTimeDrag(event, item.id, "timeline", "end", savedStart, savedEnd)} onPointerMove={moveTimeDrag} onPointerUp={endTimeDrag} onPointerCancel={endTimeDrag} disabled={disabled} aria-label={`${item.title}の予定終了時刻をドラッグで変更`} title="予定終了時刻を変更" /></div>{actualStart !== null && actualEnd !== null && <button type="button" className={`actual-gantt-bar ${item.actualEndAt ? "complete" : "running"}`} style={{ left: `${((actualStart - rangeStart) / duration) * 100}%`, width: `${Math.max(1.2, ((actualEnd - actualStart) / duration) * 100)}%` }} onClick={() => onEdit(item)} disabled={disabled} aria-label={`${item.title}の実績を編集`} title={item.actualEndAt ? formatDateTimeRange(item.actualStartAt || "", item.actualEndAt) : `${formatDateTime(item.actualStartAt || "")}から進行中`} />}</div>
      </div>;
    })}
    <div id="staffing" className="gantt-section-label"><span className="operation-icon staffing">♙</span>当日体制</div><div className="gantt-section-action" aria-hidden="true" /><div className="gantt-section-line staffing"><span>STAFFING</span></div>
    {staffingRanges.map(({ assignment, start: savedStart, end: savedEnd }) => {
      ...
      return <div className="gantt-row" key={`staffing-${assignment.id}`}><div className="gantt-label staffing-gantt-label"><button type="button" className="gantt-label-detail staffing-label-detail" onClick={() => onOpenPreview({ type: "staffing", item: assignment })} disabled={disabled} aria-label={`${assignment.name}の体制明細を表示`}><span className="member-avatar">{assignment.name.slice(0, 1)}</span><span><span className="staffing-name-line"><strong>{assignment.name}</strong><b className={`staffing-mode-badge mode-${assignment.workMode}`}>{staffingModeIcon(assignment.workMode)} {assignment.workMode}</b></span><small>{assignment.phone || "電話番号未登録"}・{assignment.location || assignment.workMode}・{formatDateTimeRange(fromMinutes(start), fromMinutes(end))}</small></span></button></div><div className="gantt-action-cell staffing-action-cell" aria-hidden="true" /><div className="gantt-lane" style={{ backgroundSize: `${100 / Math.max(1, ticks.length - 1)}% 100%` }}><div className={`staffing-gantt-bar mode-${assignment.workMode} ${dragging ? "dragging" : ""} ${disabled ? "disabled" : ""}`} style={{ left: `${((start - rangeStart) / duration) * 100}%`, width: `${((end - start) / duration) * 100}%` }}><button type="button" className="staffing-gantt-content" onPointerDown={(event) => beginTimeDrag(event, assignment.id, "staffing", "move", savedStart, savedEnd)} onPointerMove={moveTimeDrag} onPointerUp={endTimeDrag} onPointerCancel={endTimeDrag} onClick={() => { if (suppressClickRef.current) { suppressClickRef.current = false; return; } onEditStaffing(assignment); }} disabled={disabled} aria-label={`${assignment.name}の対応時間帯をドラッグで移動。クリックで編集`} title="左右にドラッグして対応時間帯を移動"><strong>{staffingModeIcon(assignment.workMode)} {assignment.workMode}・{assignment.name}</strong><span>{assignment.location || assignment.note || formatDateTimeRange(fromMinutes(start), fromMinutes(end))}</span></button><button className="gantt-resize-handle start" onPointerDown={(event) => beginTimeDrag(event, assignment.id, "staffing", "start", savedStart, savedEnd)} onPointerMove={moveTimeDrag} onPointerUp={endTimeDrag} onPointerCancel={endTimeDrag} disabled={disabled} aria-label={`${assignment.name}の対応開始時刻をドラッグで変更`} title="対応開始時刻を変更" /><button className="gantt-resize-handle end" onPointerDown={(event) => beginTimeDrag(event, assignment.id, "staffing", "end", savedStart, savedEnd)} onPointerMove={moveTimeDrag} onPointerUp={endTimeDrag} onPointerCancel={endTimeDrag} disabled={disabled} aria-label={`${assignment.name}の対応終了時刻をドラッグで変更`} title="対応終了時刻を変更" /></div></div></div>;
    })}
  </div></div>;
}
...
function CopyWorkModal({ work, saving, onClose, onSubmit }: { work: ReleaseWork; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal copy-work-modal" role="dialog" aria-modal="true" aria-labelledby="copy-work-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="section-kicker">COPY RELEASE WORK</span><h2 id="copy-work-title">作業をコピー</h2></div><button onClick={onClose} aria-label="閉じる" disabled={saving}>×</button></div><form onSubmit={onSubmit}>
    ...
    <div className="field-row"><label>プロジェクト番号（任意）<input name="projectNumber" defaultValue={work.release.projectNumber} placeholder="例：PJ-2026-001" /></label><label>環境<select name="environment" defaultValue={work.release.environment}><option>Production</option><option>Staging</option><option>Development</option></select></label></div>
...
function ItemModal({ type, editTarget, releaseDate, staffing: staffingOptions, approvals, links, approvalCategories, formError, saving, onClose, onSubmit, onRequestDelete }: { type: ModalType; editTarget: EditTarget | null; releaseDate?: string; staffing: StaffingAssignment[]; approvals: ApprovalItem[]; links: ResourceLink[]; approvalCategories: Category[]; formError: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onRequestDelete: (target: DetailTarget) => void }) {
  ...
  const [timelineKind, setTimelineKind] = useState<TimelineKind>(timeline?.kind || "作業");
  ...
  const staffingMode = normalizeStaffingMode(staffing?.workMode, staffing?.location);
  const staffingModes: StaffingMode[] = ["現地", "リモート", "オンコール", "その他"];
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="section-kicker">{editing ? "EDIT ITEM" : type === "work" ? "NEW RELEASE WORK" : "NEW ITEM"}</span><h2 id="modal-title">{title}</h2></div><button onClick={onClose} aria-label="閉じる">×</button></div><form onSubmit={onSubmit}>
    ...
    {type === "work" && <><div className="field-row"><label>SystemID<input name="systemId" defaultValue={work?.systemId} placeholder="例：PAYMENT" required autoFocus /></label><label>作業名<input name="name" defaultValue={work?.name} placeholder="例：決済基盤アップデート" required /></label></div><div className="field-row"><label>プロジェクト番号（任意）<input name="projectNumber" defaultValue={work?.projectNumber} placeholder="例：PJ-2026-001" /></label><label>環境<select name="environment" defaultValue={work?.environment || "Production"}><option>Production</option><option>Staging</option><option>Development</option></select></label></div><label>作業日時<input name="releaseDate" type="datetime-local" defaultValue={work?.releaseDate.replace(" ", "T")} required /></label><label>責任者<input name="manager" defaultValue={work?.manager} placeholder="例：田中" required /></label>{work && <label>状態<select name="status" defaultValue={work.status}><option>準備中</option><option>進行中</option><option>完了</option></select></label>}<p className="form-hint">{work ? "作業の基本情報を更新します。紐づく明細は保持されます。" : "登録後にタイムチャート・申請物・資料を追加します。"}</p></>}
    {type === "staffing" && <><div className="field-row"><label>氏名<input name="name" defaultValue={staffing?.name} placeholder="例：佐藤" required autoFocus /></label><label>電話番号<input name="phone" type="tel" defaultValue={staffing?.phone} placeholder="例：090-1234-5678" /></label></div><fieldset className="staffing-mode-fieldset"><legend>対応形態</legend><div className="staffing-mode-options">{staffingModes.map((mode) => <label className={`staffing-mode-option mode-${mode}`} key={mode}><input type="radio" name="workMode" value={mode} defaultChecked={staffingMode === mode} /><span><b>{staffingModeIcon(mode)}</b><strong>{mode}</strong></span></label>)}</div></fieldset><div className="field-row"><label>対応開始日時<input name="startAt" type="datetime-local" defaultValue={staffingStartAt} required /></label><label>対応終了日時<input name="endAt" type="datetime-local" defaultValue={staffingEndAt} required /></label></div><p className="field-help">新規追加時は当日作業の開始から8時間を初期表示します。</p><label>場所・拠点・補足（任意）<input name="location" defaultValue={staffing?.location} placeholder="例：名古屋、在宅、連絡先待機" /></label><label>役割・補足<input name="note" defaultValue={staffing?.note} placeholder="例：作業責任者、一次連絡先" /></label></>}
    {type === "timeline" && <><div className="field-row"><label>作業タイトル<input name="title" defaultValue={timeline?.title} placeholder="例：本番デプロイ" required autoFocus /></label><label>担当者<input name="owner" list="staffing-owner-options" defaultValue={timeline?.owner} placeholder="当日体制から選択または入力" required /></label><datalist id="staffing-owner-options">{staffingOptions.map((assignment) => <option value={assignment.name} key={assignment.id}>{assignment.location}</option>)}</datalist></div><label>内容（任意）<textarea name="content" defaultValue={timeline?.content} rows={3} placeholder="具体的な作業内容や確認事項" /></label><div className="field-row timeline-classification"><label>種別<select name="kind" value={timelineKind} onChange={(event) => setTimelineKind(event.target.value as TimelineKind)}><option>作業</option><option>申請物</option></select></label><label>区分<select name="plan" defaultValue={timeline?.plan || "本線"}><option>本線</option><option>コンチプラン</option></select></label><label>状態<select name="status" defaultValue={timeline?.status || "未着手"}><option>未着手</option><option>進行中</option><option>完了</option></select></label></div>{timelineKind === "申請物" && <label>紐づける申請物（任意）<select name="approvalId" defaultValue={timeline?.approvalId ? String(timeline.approvalId) : ""}><option value="">紐づけなし</option>{approvals.map((item) => <option value={item.id} key={item.id}>{item.title}（{item.status}）</option>)}</select></label>}{timelineKind === "申請物" && !approvals.length && <p className="field-help">申請物一覧へ申請を登録すると、ここから紐づけを選択できます。</p>}{timelineKind === "作業" && <label>紐づける手順書・関連リンク（任意）<select name="resourceLinkId" defaultValue={timeline?.resourceLinkId ? String(timeline.resourceLinkId) : ""}><option value="">紐づけなし</option>{links.map((item) => <option value={item.id} key={item.id}>{item.title}（{item.category}）</option>)}</select></label>}{timelineKind === "作業" && !links.length && <p className="field-help">手順書・関連リンクを登録すると、ここから紐づけを選択できます。</p>}<p className="field-help">担当者は当日体制の登録メンバーから選択できます。未登録の名前も直接入力できます。</p><fieldset className="time-fieldset"><legend>予定</legend><div className="field-row"><label>作業日<input name="workDate" type="date" defaultValue={plannedStartAt.slice(0, 10)} required /></label><label>終了日<input name="endDate" type="date" defaultValue={plannedEndAt.slice(0, 10)} required /></label></div><div className="field-row"><label>開始時刻<input name="startTime" type="time" step="300" defaultValue={plannedStartAt.slice(11, 16)} required /></label><label>終了時刻<input name="endTime" type="time" step="300" defaultValue={plannedEndAt.slice(11, 16)} required /></label></div><div className="duration-presets"><span>開始から</span>{[15, 30, 60].map((minutes) => <button type="button" key={minutes} onClick={(event) => setPlannedDuration(event.currentTarget, minutes)}>＋{minutes}分</button>)}</div><p className="field-help">作業日と開始時刻は親作業の予定日時を初期表示します。日を跨ぐ場合は終了日を変更できます。</p></fieldset><fieldset className="time-fieldset actual"><legend>実績（任意）</legend><div className="field-row"><div className="actual-datetime-field"><label>実績開始日時<input name="actualStartAt" type="datetime-local" defaultValue={timeline?.actualStartAt} /></label><button type="button" className="set-now-button" onClick={(event) => setCurrentDateTime(event.currentTarget, "actualStartAt")}>◷ 今を開始に設定</button></div><div className="actual-datetime-field"><label>実績終了日時<input name="actualEndAt" type="datetime-local" defaultValue={timeline?.actualEndAt} /></label><button type="button" className="set-now-button" onClick={(event) => setCurrentDateTime(event.currentTarget, "actualEndAt")}>◷ 今を終了に設定</button></div></div><p className="field-help">作業中は実績開始のみ入力できます。</p></fieldset></>}
    {type === "approval" && <><label>申請名<input name="title" defaultValue={approval?.title} required autoFocus /></label><label>申請種別（任意）<input name="category" list="approval-category-options" defaultValue={approval?.category} placeholder="候補から選択または手入力" /></label><datalist id="approval-category-options">{approvalCategories.map((category) => <option value={category.name} key={category.id}>{category.description}</option>)}</datalist><p className="field-help">管理画面の候補から選択できます。候補にない種別も直接入力できます。</p><div className="field-row"><label>担当者<input name="owner" defaultValue={approval?.owner} required /></label><label>状態<select name="status" defaultValue={approval?.status || "未申請"}><option>未申請</option><option>申請中</option><option>回付済</option><option>結了済</option></select></label></div><label>期限<input name="due" type="date" defaultValue={approval?.due || releaseDate?.slice(0, 10)} required /></label><label>備考（任意）<textarea name="note" defaultValue={approval?.note} rows={3} placeholder="申請時の注意点や補足" /></label><UrlInputField label="申請リンク（任意）" defaultValue={approval?.url} /></>}
    {type === "link" && <><label>タイトル<input name="title" defaultValue={link?.title} required autoFocus /></label><label>カテゴリ<input name="category" defaultValue={link?.category} placeholder="手順書" required /></label><label>説明<input name="description" defaultValue={link?.description} required /></label><label>備考（任意）<textarea name="note" defaultValue={link?.note} rows={3} placeholder="利用時の注意点や補足" /></label><UrlInputField label="URL（任意）" defaultValue={link?.url} /></>}
    <div className="modal-actions modal-actions-split">{editTarget && editTarget.type !== "work" ? <button type="button" className="danger-button" onClick={() => onRequestDelete(editTarget)} disabled={saving}>この明細を削除</button> : <span />}<span><button type="button" className="ghost-button" onClick={onClose}>キャンセル</button><button type="submit" className="primary-button" disabled={saving}>{saving ? "保存中" : editing ? "変更を保存" : type === "work" ? "登録して明細へ" : "追加する"}</button></span></div>
  </form></div></div>;
}

function UrlInputField({ label, defaultValue }: { label: string; defaultValue?: string }) {
  ...
  async function pasteUrl() {
    try {
      ...
      setMessage(value ? "クリップボードから貼り付けました" : "クリップボードは空です");
    } catch {
      setMessage("クリップボードを読み取れませんでした");
    }
  }
  function encodeUrl() {
    ...
    if (!input) return;
    try {
      input.value = encodeURI(decodeURI(input.value));
    } catch {
      input.value = encodeURI(input.value);
    }
    setMessage("URLをエンコードしました");
  }
  return <div className="url-input-field"><label>{label}<input ref={inputRef} name="url" inputMode="url" defaultValue={defaultValue} placeholder="後から登録できます" /></label><div className="url-tool-actions"><button type="button" onClick={() => void pasteUrl()}>クリップボードからペースト</button><button type="button" onClick={encodeUrl}>URLエンコード</button>{message && <span role="status">{message}</span>}</div></div>;
}

function PreviewModal({ preview, onClose, onEdit, onRequestDelete }: { preview: PreviewItem; onClose: () => void; onEdit: (target: EditTarget) => void; onRequestDelete: (target: DetailTarget) => void }) {
  const title = preview.type === "staffing" ? preview.item.name : preview.item.title;
  const kicker = preview.type === "timeline" ? "TIMELINE DETAIL" : preview.type === "staffing" ? "STAFFING DETAIL" : preview.type === "approval" ? "APPROVAL DETAIL" : "RESOURCE DETAIL";
  const externalUrl = preview.type === "approval" || preview.type === "link" ? preview.item.url : "";
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal preview-modal" role="dialog" aria-modal="true" aria-labelledby="preview-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="section-kicker">{kicker}</span><h2 id="preview-title">{title}</h2></div><button onClick={onClose} aria-label="閉じる">×</button></div><div className="preview-body">
    {preview.type === "timeline" ? <><div className="preview-status"><span className={`tag status-${preview.item.status}`}>{preview.item.status}</span><span className="preview-category">{preview.item.kind || "作業"}・{preview.item.plan}</span></div>{preview.item.content && <p>{preview.item.content}</p>}<dl><div><dt>担当者</dt><dd>{preview.item.owner}</dd></div><div><dt>予定</dt><dd>{formatDateTimeRange(preview.item.startAt, preview.item.endAt)}</dd></div><div><dt>実績</dt><dd>{preview.item.actualStartAt ? preview.item.actualEndAt ? formatDateTimeRange(preview.item.actualStartAt, preview.item.actualEndAt) : `${formatDateTime(preview.item.actualStartAt)}–進行中` : "未入力"}</dd></div></dl></>
      : preview.type === "staffing" ? <><dl><div><dt>対応形態</dt><dd><span className={`staffing-mode-badge mode-${preview.item.workMode}`}>{staffingModeIcon(preview.item.workMode)} {preview.item.workMode}</span></dd></div><div><dt>電話番号</dt><dd>{preview.item.phone || "未登録"}</dd></div><div><dt>場所・拠点</dt><dd>{preview.item.location || preview.item.workMode}</dd></div><div><dt>対応時間</dt><dd>{formatDateTimeRange(preview.item.startAt, preview.item.endAt)}</dd></div></dl>{preview.item.note && <div className="preview-note"><strong>備考</strong><p>{preview.item.note}</p></div>}</>
      : preview.type === "approval" ? <><div className="preview-status"><span className={`tag status-${preview.item.status}`}>{preview.item.status}</span>{preview.item.category && <span className="preview-category">{preview.item.category}</span>}</div><dl><div><dt>担当者</dt><dd>{preview.item.owner}</dd></div><div><dt>期限</dt><dd>{formatDueDate(preview.item.due)}</dd></div></dl>{preview.item.note && <div className="preview-note"><strong>備考</strong><p>{preview.item.note}</p></div>}</>
      : <><span className="preview-category">{preview.item.category}</span><p>{preview.item.description}</p>{preview.item.note && <div className="preview-note"><strong>備考</strong><p>{preview.item.note}</p></div>}</>}
    <div className="modal-actions modal-actions-split"><button type="button" className="danger-button" onClick={() => onRequestDelete(preview)}>削除</button><span><button type="button" className="ghost-button" onClick={() => onEdit(preview)}>情報を編集</button>{(preview.type === "approval" || preview.type === "link") && (externalUrl ? <a className="primary-button modal-link-button" href={externalUrl} target="_blank" rel="noopener noreferrer">そのまま開く ↗</a> : <span className="link-unregistered">リンク未登録</span>)}</span></div>
  </div></div></div>;
}

function DetailDeleteConfirmModal({ target, saving, onClose, onConfirm }: { target: DetailTarget; saving: boolean; onClose: () => void; onConfirm: () => void }) {
  const typeName = target.type === "timeline" ? "タイムチャート" : target.type === "staffing" ? "当日体制" : target.type === "approval" ? "申請物" : "手順書・関連リンク";
  const title = target.type === "staffing" ? target.item.name : target.item.title;
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal delete-confirm-modal" role="alertdialog" aria-modal="true" aria-labelledby="detail-delete-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="section-kicker danger">DELETE DETAIL</span><h2 id="detail-delete-title">{typeName}を削除しますか？</h2></div><button onClick={onClose} aria-label="閉じる" disabled={saving}>×</button></div><div className="delete-confirm-body"><strong>{title}</strong><p>この明細を削除します。申請物を削除した場合、タイムチャートとの紐づけも解除されます。この操作は取り消せません。</p></div><div className="modal-actions"><button type="button" className="ghost-button" onClick={onClose} disabled={saving}>キャンセル</button><button type="button" className="danger-button solid" onClick={onConfirm} disabled={saving}>{saving ? "削除中" : "削除する"}</button></div></div></div>;
}
