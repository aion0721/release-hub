...
export default function App() {
  ...
  return (
    <main className="app-shell">
      ...
      {modal && <ItemModal type={modal} editTarget={editTarget} releaseDate={selected?.release.releaseDate} staffing={selected?.staffing || []} approvals={selected?.approvals || []} approvalCategories={approvalCategories} formError={formError} saving={saving} onClose={closeModal} onSubmit={submitItem} />}
...
function linkedApprovalName(item: TimelineItem, approvals: ApprovalItem[]) {
  return item.kind === "申請物" && item.approvalId ? approvals.find((approval) => approval.id === item.approvalId)?.title : undefined;
}

function TimelineList({ items, approvals, disabled, onEdit, onReorder, onStatusChange }: { items: TimelineItem[]; approvals: ApprovalItem[]; disabled: boolean; onEdit: (item: TimelineItem) => void; onReorder: (sourceId: number, targetId: number | null, targetPlan: TimelinePlan) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
  ...
  return <div className="timeline-list">{plans.map((plan) => {
    ...
    return <section className={`timeline-group plan-${plan}`} key={plan} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const sourceId = Number(event.dataTransfer.getData("text/plain")) || draggingId; if (sourceId) onReorder(sourceId, null, plan); setDraggingId(null); }}><div className="timeline-group-heading"><h3>{plan}</h3><span>{planItems.length}件</span></div>
      {planItems.length ? planItems.map((item, index) => { const approvalName = linkedApprovalName(item, approvals); return <div key={item.id} draggable={!disabled} className={`timeline-row ${item.status === "完了" ? "done" : ""} ${draggingId === item.id ? "dragging" : ""}`} onClick={() => onEdit(item)} onKeyDown={(event) => { if (event.key === "Enter") onEdit(item); }} onDragStart={(event: DragEvent<HTMLDivElement>) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", String(item.id)); setDraggingId(item.id); }} onDragEnd={() => setDraggingId(null)} onDragOver={(event: DragEvent<HTMLDivElement>) => event.preventDefault()} onDrop={(event: DragEvent<HTMLDivElement>) => { event.preventDefault(); event.stopPropagation(); const sourceId = Number(event.dataTransfer.getData("text/plain")) || draggingId; if (sourceId) onReorder(sourceId, item.id, plan); setDraggingId(null); }} role="button" tabIndex={disabled ? -1 : 0} aria-label={`${item.title}を編集`} title="ドラッグして並べ替え。クリックで編集"><span className="drag-handle">⋮⋮</span><span className="time">{formatDateTime(item.startAt)}</span><span className="line"><i>{item.status === "完了" ? "✓" : index + 1}</i></span><span className="task"><strong><b className={`timeline-kind kind-${item.kind || "作業"}`}>{item.kind || "作業"}</b>{item.title}</strong>{item.content && <small className="timeline-content">{item.content}</small>}{approvalName && <small className="timeline-approval-link">申請物：{approvalName}</small>}<small><b className="time-kind planned">予定</b>担当：{item.owner}・{formatDateTimeRange(item.startAt, item.endAt)}</small><small className={`actual-time ${item.actualStartAt ? "recorded" : ""}`}><b className="time-kind actual">実績</b>{item.actualStartAt ? item.actualEndAt ? formatDateTimeRange(item.actualStartAt, item.actualEndAt) : `${formatDateTime(item.actualStartAt)}–進行中` : "未入力"}</small></span><span className="timeline-row-actions"><span className={`tag status-${item.status}`}>{item.status}</span>{item.status === "未着手" && <button type="button" className="timeline-action start" disabled={disabled} onClick={(event) => { event.stopPropagation(); onStatusChange(item.id, "start"); }}>▶ 開始</button>}{item.status === "進行中" && <button type="button" className="timeline-action complete" disabled={disabled} onClick={(event) => { event.stopPropagation(); onStatusChange(item.id, "complete"); }}>✓ 完了</button>}</span></div>; }) : <p className="timeline-group-empty">ここにドロップして移動</p>}
    </section>;
  })}</div>;
}
...
function AllInOneList({ items, assignments, approvals, disabled, onEditTimeline, onEditStaffing, onReorder, onStatusChange }: { items: TimelineItem[]; assignments: StaffingAssignment[]; approvals: ApprovalItem[]; disabled: boolean; onEditTimeline: (item: TimelineItem) => void; onEditStaffing: (item: StaffingAssignment) => void; onReorder: (sourceId: number, targetId: number | null, targetPlan: TimelinePlan) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
  return <div className="all-in-one-list">
    <section className="operation-list-section"><div className="operation-subheading"><div><span className="operation-icon work">◷</span><h3>タイムチャート</h3></div><span>{items.length}件</span></div><TimelineList items={items} approvals={approvals} disabled={disabled} onEdit={onEditTimeline} onReorder={onReorder} onStatusChange={onStatusChange} /></section>
...
function GanttChart({ items, assignments, approvals, disabled, onEdit, onEditStaffing, onTimeChange, onStaffingTimeChange, onStatusChange }: { items: TimelineItem[]; assignments: StaffingAssignment[]; approvals: ApprovalItem[]; disabled: boolean; onEdit: (item: TimelineItem) => void; onEditStaffing: (item: StaffingAssignment) => void; onTimeChange: (id: number, startAt: string, endAt: string) => void; onStaffingTimeChange: (id: number, startAt: string, endAt: string) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
...
function ItemModal({ type, editTarget, releaseDate, staffing: staffingOptions, approvals, approvalCategories, formError, saving, onClose, onSubmit }: { type: ModalType; editTarget: EditTarget | null; releaseDate?: string; staffing: StaffingAssignment[]; approvals: ApprovalItem[]; approvalCategories: Category[]; formError: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
