...
export default function App() {
  ...
  return (
    <main className="app-shell">
      ...
      {modal && <ItemModal type={modal} editTarget={editTarget} releaseDate={selected?.release.releaseDate} staffing={selected?.staffing || []} approvals={selected?.approvals || []} approvalCategories={approvalCategories} formError={formError} saving={saving} onClose={closeModal} onSubmit={submitItem} onRequestDelete={requestDetailDelete} />}
...
function TimelineList({ items, approvals, disabled, onEdit, onOpenApproval, onReorder, onStatusChange }: { items: TimelineItem[]; approvals: ApprovalItem[]; disabled: boolean; onEdit: (item: TimelineItem) => void; onOpenApproval: (item: ApprovalItem) => void; onReorder: (sourceId: number, targetId: number | null, targetPlan: TimelinePlan) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
...
function AllInOneList({ items, assignments, approvals, disabled, onEditTimeline, onEditStaffing, onOpenApproval, onReorder, onStatusChange }: { items: TimelineItem[]; assignments: StaffingAssignment[]; approvals: ApprovalItem[]; disabled: boolean; onEditTimeline: (item: TimelineItem) => void; onEditStaffing: (item: StaffingAssignment) => void; onOpenApproval: (item: ApprovalItem) => void; onReorder: (sourceId: number, targetId: number | null, targetPlan: TimelinePlan) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
  return <div className="all-in-one-list">
    <section className="operation-list-section"><div className="operation-subheading"><div><span className="operation-icon work">◷</span><h3>タイムチャート</h3></div><span>{items.length}件</span></div><TimelineList items={items} approvals={approvals} disabled={disabled} onEdit={onEditTimeline} onOpenApproval={onOpenApproval} onReorder={onReorder} onStatusChange={onStatusChange} /></section>
...
function GanttChart({ items, assignments, approvals, disabled, onEdit, onEditStaffing, onOpenApproval, onTimeChange, onStaffingTimeChange, onStatusChange }: { items: TimelineItem[]; assignments: StaffingAssignment[]; approvals: ApprovalItem[]; disabled: boolean; onEdit: (item: TimelineItem) => void; onEditStaffing: (item: StaffingAssignment) => void; onOpenApproval: (item: ApprovalItem) => void; onTimeChange: (id: number, startAt: string, endAt: string) => void; onStaffingTimeChange: (id: number, startAt: string, endAt: string) => void; onStatusChange: (id: number, action: "start" | "complete") => void }) {
...
function ItemModal({ type, editTarget, releaseDate, staffing: staffingOptions, approvals, approvalCategories, formError, saving, onClose, onSubmit, onRequestDelete }: { type: ModalType; editTarget: EditTarget | null; releaseDate?: string; staffing: StaffingAssignment[]; approvals: ApprovalItem[]; approvalCategories: Category[]; formError: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onRequestDelete: (target: DetailTarget) => void }) {
...
function PreviewModal({ preview, onClose, onEdit, onRequestDelete }: { preview: PreviewItem; onClose: () => void; onEdit: (target: EditTarget) => void; onRequestDelete: (target: DetailTarget) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal preview-modal" role="dialog" aria-modal="true" aria-labelledby="preview-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="section-kicker">{isApproval ? "APPROVAL DETAIL" : "RESOURCE DETAIL"}</span><h2 id="preview-title">{item.title}</h2></div><button onClick={onClose} aria-label="閉じる">×</button></div><div className="preview-body">
    {isApproval ? <><div className="preview-status"><span className={`tag status-${preview.item.status}`}>{preview.item.status}</span>{preview.item.category && <span className="preview-category">{preview.item.category}</span>}</div><dl><div><dt>担当者</dt><dd>{preview.item.owner}</dd></div><div><dt>期限</dt><dd>{formatDueDate(preview.item.due)}</dd></div></dl>{preview.item.note && <div className="preview-note"><strong>備考</strong><p>{preview.item.note}</p></div>}</> : <><span className="preview-category">{preview.item.category}</span><p>{preview.item.description}</p>{preview.item.note && <div className="preview-note"><strong>備考</strong><p>{preview.item.note}</p></div>}</>}
    <div className="modal-actions modal-actions-split"><button type="button" className="danger-button" onClick={() => onRequestDelete(preview)}>削除</button><span><button type="button" className="ghost-button" onClick={() => onEdit(preview)}>情報を編集</button>{item.url ? <a className="primary-button modal-link-button" href={item.url} target="_blank" rel="noopener noreferrer">そのまま開く ↗</a> : <span className="link-unregistered">リンク未登録</span>}</span></div>
  </div></div></div>;
}
